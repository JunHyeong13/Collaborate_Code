...
class MocapMotionAnalyzer:
    ...
    def __init__(self, scaling: str = 'standard', feature_weights: dict | None = None,
        ...
        # 파트 가중치 초기화 (동등 가중)
        self.part_weights: dict[str, float] = {
            'part_left_arm': 1.0,
            'part_right_arm': 1.0,
            'part_left_leg': 1.0,
            'part_right_leg': 1.0,
    ...
    def set_part_weights(self, part_weights: dict[str, float], normalize: bool = True):
        """파트 가중치를 동적으로 설정합니다. 키는 'part_left_arm' 형태를 사용하세요."""
        if not part_weights:
            return
        # 허용 키만 업데이트
        for k, v in part_weights.items():
            if k in self.part_weights:
                self.part_weights[k] = float(v)
        if normalize:
            s = sum(self.part_weights.values())
            if s <= 0:
                s = 1.0
            for k in list(self.part_weights.keys()):
                self.part_weights[k] = self.part_weights[k] / s
    ...
    def compare_motions(self, motion1_data: pd.DataFrame, motion2_data: pd.DataFrame):
        ...
        try:
            ...
            for part_name, part_joints in parts.items():
                ...
                # 모든 피처를 고려한 파트 스코어(사용자 가중치 적용 평균)
                per_feature_scores = {}
                weights_used = []
                weighted_vals = []
                for f_type in ('position', 'rotation', 'velocity', 'acceleration', 'joint_angles'):
                    score = part_feature_score(f_type, jset, part_name)
                    per_feature_scores[f_type] = score
                    w = float(self.feature_weights.get(f_type, 0.0))
                    if w > 0.0:
                        weights_used.append(w)
                        weighted_vals.append(score * w)
                part_breakdown[part_name] = per_feature_scores
                vals = list(per_feature_scores.values())
                # 가중치 합이 0이면 동일 가중 평균으로 폴백
                part_scores[part_name] = (
                    float(np.sum(weighted_vals) / np.sum(weights_used)) if weights_used else
            ...
            # 최종 유사도: 파트 점수의 가중 평균(파트 점수는 이미 특성 가중치 반영됨)
            part_weight_list = []
            part_weighted_vals = []
            for pk, pv in part_scores.items():
                w = float(self.part_weights.get(f"part_{pk}", self.part_weights.get(pk, 0.0)))
                if w > 0.0:
                    part_weight_list.append(w)
                    part_weighted_vals.append(pv * w)
            if part_weight_list:
                final_similarity = float(np.sum(part_weighted_vals) / np.sum(part_weight_list))
            else:
                # 폴백: 특성 가중 합(이전 방식)
            ...
            return float(final_similarity), {
                ...
                **{f"part_{k}": v for k, v in part_scores.items()},
                ...
                'part_weights': self.part_weights.copy(),
            }
